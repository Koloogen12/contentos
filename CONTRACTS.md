...
### Ведение запуска (миграция 0018)

Слоты правятся, а не только размечаются. `version` обязателен при любой
правке: слот открывают из нескольких вкладок, и молча перетирать чужое нельзя
— расхождение отдаётся `409` с объяснением, что делать.

```
POST   /api/v1/launches/{id}/slots                  добавить слот руками (сразу закреплён)
PATCH  /api/v1/launches/{id}/slots/{slot_id}        разметка, перенос, отметки публикации
DELETE /api/v1/launches/{id}/slots/{slot_id}        убрать слот (опубликованный — 409)
POST   /api/v1/launches/{id}/slots/confirm          подтвердить разметку этапа целиком
```

Подтверждение этапа — отдельная ручка, потому что в этапе бывает сорок слотов:
сорок PATCH подряд дали бы сорок гонок за версию. Возвращает число фактически
подтверждённых.

Перенос слота меняет этап вместе с датой: дата вне оси запуска отдаётся `400`,
потому что такой слот не показать ни на одном экране и не посчитать ни одной
квотой.

`PATCH` слота принимает: `scheduled_date`, `rubric`, `meaning`, `checkpoints`,
`trigger_key`, `knowledge_item_id`, `talking_point_text`, `has_proof`,
`is_peak`, `is_pinned`, `status`, `reaction` (1–3), `draft_state`
(`writing`/`ready`), `chars`, `full_text`, `confirm`, `version`.

### Фактура и задачи

```
GET    /api/v1/launches/{id}/evidence               состояние всех сорока смыслов
PATCH  /api/v1/launches/{id}/evidence/{meaning_key} proof · claimed · none
GET    /api/v1/launches/{id}/tasks                  задачи на добычу
```

`GET /evidence` отдаёт все сорок смыслов, а не только сохранённые: отсутствие
строки означает «нечем доказать», и клиент не должен догадываться об этом сам.

Задачи не хранятся и не создаются. Задача — это смысл в состоянии `none` плюс
дедлайн, посчитанный за два дня до первого слота, где смысл понадобится.
Закрыл смысл пруфом — задача исчезла сама. Персистится ровно одно поле,
которое не выводится: `task_dismissed`.

### Сюжетные линии: обещание живёт постом

`PATCH /story-lines/{line_id}` принимает `announce_slot_id` и `close_slot_id`.
Даты линии следуют за постами, а не наоборот. `is_closed` снаружи не задаётся:
это следствие — линия закрыта тогда и только тогда, когда под раскрытие стоит
конкретный слот и он не позже закрытия продаж. Иначе можно было бы пометить
закрытым обещание, которого никто не увидит.

### Режим запуска не хранится

`LaunchOut.mode` — `draft` · `warm` · `sales` · `closed` — считается из
`unrolled_on`, `sales_open`, `sales_close` и `archived_at`. Колонки под него
нет намеренно: хранимая копия расходится с датами на первом же переносе окна.
Клиенту отдаётся готовым, чтобы четыре места в интерфейсе не вывели его
четырьмя способами.

Новые поля рамки: `price`, `audience`, `collect`, `waitlist`, `paid`,
`paid_goal`, `unrolled_on`. Интенсивность принимает `dense` как синоним
`heavy` — имя из прототипа, чтобы клиент не переводил туда-обратно.

### Слот: рубрика отдельно от смысла

До 0018 рубрика лежала в колонке `meaning`, а смыслы покупателя в массиве
`checkpoints`, и читалось это ровно наоборот методологии. Теперь двенадцать
конструкторских рубрик — `rubric`, сорок смыслов — `meaning`. Данные перенесены
миграцией, а не сброшены. Добавлены `draft_state`, `chars`, `reaction`,
`line_role`, `empty_reason`; статус слота принимает `missed` — «день прошёл,
пост не вышел», в отличие от `skipped`, который решение автора.
